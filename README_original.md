...
├── app_api.py              # FastAPI backend entry point
├── app_streamlit.py        # Streamlit UI entry point
├── start_fullstack.ps1     # PowerShell script to start both backend and frontend
├── LICENSE
├── README.md
...
## Setup Instruction
There are 3 entry points in the application: 1. FastAPI Backend 2. Streamlit UI 3. React Frontend (recommended). 
...
6. Pull the Redis image: `docker pull redis`
7. Run the Redis container: `docker run -p 6379:6379 redis` (Docker is used to store the state of the graph in Redis)
8. Set up environment variables in .env file with your API keys

### Frontend Setup (React - Recommended)
...
- **Streamlit UI**: Run `streamlit run app_streamlit.py`
- **FastAPI only**: Run `python app_api.py` (backend API only)

### Quick Start (Full Stack)
Use the PowerShell script to start both backend and frontend together:
```powershell
.\start_fullstack.ps1
```
